...
const App = () => {
  const [authenticate, setauthenticate] = useState({ authenticate: false, user: false })
  useEffect(() => {

    const token = localStorage.getItem("token")
    if (token) {
      axios.post("http://localhost:80/verify/", ({ token: token })).then((res) => {
        console.log(res)
  ...
  return (
    ...
    <Router>
      <MainNavigation logout={logout} user={authenticate.user} loginisvalid={authenticate.authenticate} />
      <main>
        {routes}
      </main>
